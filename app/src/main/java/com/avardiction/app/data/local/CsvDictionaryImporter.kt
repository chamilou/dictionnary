    ...
    fun import(
        ...
        context.assets.open(assetName).bufferedReader(Charsets.UTF_8).use { reader ->
            ...
            reader.lineSequence()
                ...
                .forEach { line ->
                    processedRows += 1
                    if (processedRows == 1 || processedRows % 250 == 0) {
                        onProgress?.invoke(ImportProgress(processedRows = processedRows))
                    }
                    ...
                    if (!shouldIndexAvarHeadword(avar)) {
                        return@forEach
                    }
                    ...
                    if (avar.isNotBlank()) {
                        val normalizedAvar = SearchNormalizer.normalize(avar, "av")
                        translations += TranslationEntity(
                            entryId = nextEntryId,
                            languageCode = "av",
                            text = avar,
                            normalizedText = normalizedAvar,
                            browseKey = SearchNormalizer.extractBrowseLetter(avar, "av"),
                            isPrimary = true,
                    ...
                    if (russian.isNotBlank()) {
                        ...
                        translations += TranslationEntity(
                            ...
                            languageCode = "ru",
                            text = russian,
                            normalizedText = normalizedRussian,
                            browseKey = SearchNormalizer.extractBrowseLetter(russian, "ru"),
                            isPrimary = true,
                    ...
                    if (english.isNotBlank()) {
                        val normalizedEnglish = SearchNormalizer.normalize(english, "en")
                        translations += TranslationEntity(
                            entryId = nextEntryId,
                            languageCode = "en",
                            text = english,
                            normalizedText = normalizedEnglish,
                            browseKey = SearchNormalizer.extractBrowseLetter(english, "en"),
                            isPrimary = true,
...
internal fun shouldIndexAvarHeadword(avar: String): Boolean {
    val trimmed = avar.trimStart()
    if (trimmed.isBlank()) {
        return true
    }
